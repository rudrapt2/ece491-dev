...
extern char _dtextc_start[];
extern char _dtextc_end[];
int dbfile;
...
#define rdint(indxfile) \
    (ch = fgetc(indxfile), \
     ((ch > 127) ? (ch - 256) : (ch)) * 256 + fgetc(indxfile))
...
/* Save location of start of message text */
    if ((indxfile_pos = ftell(indxfile)) < 0) {
        printf("Error on ftell\n");
        exit(-1);
    }
    rmsg_1.mrloc = indxfile_pos;
