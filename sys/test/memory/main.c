...
void exec_init() {
	...
	struct page_chunk** chunklist_ptr = (void*)0x8009d680; //Generated by checking address of free_chunk_list via gdb.
	unsigned long max_free = test_init(*chunklist_ptr, &fail, &total);
	unsigned long chunks = 0;
	for(struct page_chunk* curr = *chunklist_ptr; curr != NULL; curr = curr->next){
		chunks++;
	}
	...
	unsigned long chunks_cmp = 0;
	for(struct page_chunk* curr = *chunklist_ptr; curr != NULL; curr = curr->next){
		chunks_cmp++;
	}
	int will_coalesce = (chunks == chunks_cmp);
