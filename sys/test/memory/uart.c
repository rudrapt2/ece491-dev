...
struct ringbuf {
    unsigned volatile int hpos; // head of queue (from where elements are removed)
    unsigned volatile int tpos; // tail of queue (where elements are inserted)
...
long uart_read(struct io * io, void * buf, long bufsz) {
    ...
    while (rbuf_empty(&uart->rxbuf)); //Spin wait so we don't trigger a context
				      //switch and return to the main memory
				      //space
...
long uart_write(struct io * io, const void * buf, long buflen) {
    ...
    while (n < buflen) {
        ...
        while (rbuf_full(&uart->txbuf)); //Spin wait to stay in our memory space
...
void uart_isr(int srcno, void * aux) {
    ...
    if (line_status & LSR_DR) {
        if (!rbuf_full(&uart->rxbuf)) {
            rbuf_putc(&uart->rxbuf, uart->regs->rbr); //No need to condition broadcast
        } else
            uart->regs->ier &= ~IER_DRIE;
    }
    ...
    if (line_status & LSR_THRE) {
        if (!rbuf_empty(&uart->txbuf)) {
            uart->regs->thr = rbuf_getc(&uart->txbuf); //No need to condition broadcast
        } else
            uart->regs->ier &= ~IER_THREIE;
    }
#endif
