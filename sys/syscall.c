...
int64_t syscall(const struct trap_frame * tfr) {
    ...
    case SYSCALL_DEVOPEN:
        return sysdevopen(tfr->a0, (char*)tfr->a1, tfr->a2);
    case SYSCALL_FSOPEN:
        return sysfsopen(tfr->a0, (char*)tfr->a1);
    case SYSCALL_CLOSE:
...
/**
* @brief Opens unique device instance for process
* @details gets current process, allocates file descriptor (if fd = -1) or uses valid file descriptor given, validates name of device, calls open device
    ...
    if (fd < 0) {
        ...
        while (fd < PROCESS_IOMAX)
            if (proc->iotab[fd] == NULL)
                goto sysdevopen_fd_ok;
            else
                fd += 1;
        return -EMFILE;
    
    } else if (proc->iotab[fd] != NULL)
        return -EBADFD;
...
sysdevopen_fd_ok:
    ...
    result = open_device(name, instno, &proc->iotab[fd]);
...
/**
* @brief Opens file in filesystem for process
* @details gets current process, allocates file descriptor (if fd = -1) or uses valid file descriptor given, calls filesystem open
* @param fd file descriptor number
...
int sysfsopen(int fd, const char * name) {
    ...
    if (fd < 0) {
        ...
        while (fd < PROCESS_IOMAX)
            if (self->iotab[fd] == NULL)
                goto sysfsopen_fd_ok;
            else
                fd += 1;
        return -EMFILE;
    
    } else if (self->iotab[fd] != NULL)
        return -EBADFD;
...
int sysioctl(int fd, int cmd, void * arg) {
    ...
    if (cmd < 0)
        return ioctl(self->iotab[fd], cmd, arg);
    ...
    if (*wfdptr < 0) {
        *wfdptr = 0;
        while (*wfdptr < PROCESS_IOMAX)
            if (self->iotab[*wfdptr] == NULL)
                goto syspipeopen_wfd_ok;
            else
                *wfdptr += 1;
        return -EMFILE;
    
    } else if (self->iotab[*wfdptr] != NULL)
        return -EBADFD;
    ...
    if (*rfdptr < 0) {
        *rfdptr = 0;
        while (*rfdptr < PROCESS_IOMAX)
            if (self->iotab[*rfdptr] == NULL && *rfdptr != *wfdptr)
                goto syspipeopen_rfd_ok;
            else
                *rfdptr += 1;
        return -EMFILE;

} else if (self->iotab[*rfdptr] != NULL && *rfdptr != *wfdptr)
    return -EBADFD;

syspipeopen_rfd_ok:

    create_pipe(&self->iotab[*wfdptr], &self->iotab[*rfdptr]);
    return 0;
...
int sysiodup(int oldfd, int newfd) {
    ...
    if (newfd < 0) {
        ...
        while (newfd < PROCESS_IOMAX)
            if (self->iotab[newfd] == NULL)
                goto sysdup_fd_ok;
            else
                newfd += 1;
        return -EMFILE;
    
    } else if (self->iotab[newfd] != NULL)
        return -EBADFD;
